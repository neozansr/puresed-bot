...
def run_bot(idle_stage, keys_path = keys_path, config_params_path = config_params_path, transcations_df_path = transcations_df_path, open_orders_df_path = open_orders_df_path):
    ...
    open_orders_df, transcations_df = check_orders_statue(exchange, 'buy', symbol, grid, latest_price, fee_percent, open_orders_df, transcations_df)
    time.sleep(idle_stage)
    open_orders_df, transcations_df = check_orders_statue(exchange, 'sell', symbol, grid, latest_price, fee_percent, open_orders_df, transcations_df)
    time.sleep(idle_stage)
