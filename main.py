...
def run_bot(idle_stage, keys_path = keys_path, config_params_path = config_params_path, transcations_df_path = transcations_df_path, open_orders_df_path = open_orders_df_path):
    ...
    open_orders_df, transcations_df = check_orders_statue(exchange, 'buy', symbol, grid, value, latest_price, fee_percent, open_orders_df, transcations_df)
    time.sleep(idle_stage)
    open_orders_df, transcations_df = check_orders_statue(exchange, 'sell', symbol, grid, value, latest_price, fee_percent, open_orders_df, transcations_df)
    time.sleep(idle_stage)
    n_order, n_sell_order, n_open_order = cal_n_order(open_orders_df, budget, value)
    open_orders_df, transcations_df = open_buy_orders(exchange, n_order, n_sell_order, n_open_order, symbol, grid, value, latest_price, min_price, max_price, open_orders_df, transcations_df)
