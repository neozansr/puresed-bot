...
def run_bot(idle_stage, keys_path, config_params_path = config_params_path, open_orders_df_path = open_orders_df_path, transactions_df_path = transactions_df_path, assets_df_path = assets_df_path, error_log_df_path = error_log_df_path):
    ...
    symbol, budget, grid, value, min_price, max_price, start_safety, decimal = get_config_params(config_params_path)
    ...
    open_buy_orders(exchange, n_order, n_sell_order, n_open_order, symbol, grid, value, min_price, max_price, start_safety, decimal, open_orders_df_path, transactions_df_path, update_error_log)
