...
from func_order import check_orders_status, open_buy_orders, update_error_log, check_circuit_breaker, check_cut_loss, reinvest
...
def run_bot(idle_stage, idle_loop, idle_rest, keys_path, config_params_path = config_params_path, last_loop_path = last_loop_path, open_orders_df_path = open_orders_df_path, transactions_df_path = transactions_df_path, assets_df_path = assets_df_path, error_log_df_path = error_log_df_path, cash_flow_df_path = cash_flow_df_path):
    ...
    cont_flag = check_circuit_breaker(bot_name, exchange, symbol, base_currency, quote_currency, last_price, circuit_limit, idle_stage, idle_rest, last_loop_path, open_orders_df_path, transactions_df_path, error_log_df_path)

    if cont_flag == 1:
        check_cut_loss(exchange, bot_name, symbol, quote_currency, grid, last_price, config_params_path, last_loop_path, open_orders_df_path, cash_flow_df_path, idle_stage)
        ...
        print_hold_assets(symbol, base_currency, quote_currency, grid, last_price, open_orders_df_path)
        print_current_balance(exchange, symbol, quote_currency, last_price)

    reinvest(exchange, bot_name, reinvest_ratio, init_budget, budget, symbol, grid, value, n_order, last_price, config_params_path, last_loop_path, open_orders_df_path, transactions_df_path, cash_flow_df_path)
