...
def run_bot(idle_stage, keys_path, config_params_path = config_params_path, open_orders_df_path = open_orders_df_path, transactions_df_path = transactions_df_path, assets_df_path = assets_df_path, error_log_df_path = error_log_df_path):
    ...
    check_orders_status(exchange, bot_name, 'buy', symbol, grid, fee_percent, open_orders_df_path, transactions_df_path, update_error_log)
    time.sleep(idle_stage)
    check_orders_status(exchange, bot_name, 'sell', symbol, grid, fee_percent, open_orders_df_path, transactions_df_path, update_error_log)
    time.sleep(idle_stage)
    ...
    open_buy_orders(exchange, n_order, n_sell_order, n_open_order, symbol, grid, value, fee_percent, min_price, max_price, start_safety, open_orders_df_path, transactions_df_path, update_error_log)
