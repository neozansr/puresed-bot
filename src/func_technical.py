...
def find_n_positions(symbols, config_params_path):
    '''
    Find maximum number of position to open.
    '''
    config_params = get_json(config_params_path)
    n_orders = config_params['risk'] * 100
    n_symbols = len(symbols)

    n_positions = min(n_orders, n_symbols)
...
def check_open_position(exchange, config_params_path, last_loop_path, transactions_df_path):
    ...
    Each coin, Check how many positions can be opened?
        ...
        Update positions
    '''
    symbols = get_no_position_symbols(config_params_path, last_loop_path)
    n_positions = find_n_positions(symbols, config_params_path)
